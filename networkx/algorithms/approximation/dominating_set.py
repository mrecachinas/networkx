...
A `dominating set`_ for an undirected graph *G* with node set *V* and
edge set *E* is a subset *D* of *V* such that every node not in *D* is
adjacent to at least one member of *D*. An `edge dominating set`_ is a
subset *F* of *E* such that every edge not in *F* is incident to an
endpoint of at least one edge in *F*.
